...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_chauffeur_id ON chat_messages(chauffeur_id)").run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)").run(),console.log("✅ Table chat_messages prête avec colonnes de traduction")}catch(r){console.error("⚠️ Erreur auto-migration chat_messages:",r.message)}}p.post("/api/chauffeur/chat",async t=>{try{await Br(t.env.DB);const{chauffeur_id:r,message:s,sender:i,lang:a}=await t.req.json();if(!r||!s)return t.json({success:!1,error:"Données manquantes"},400);let n=a||"fr";if(!a){const u=await t.env.DB.prepare(`
      ...
      `).bind(r).first();n=(u==null?void 0:u.langue)||"fr"}const l=i||"chauffeur";console.log(`📝 [CHAT] Message reçu - Sender: ${l}, Langue: ${n} (source: ${a?"frontend":"DB"})`);let o=s,c=s,d=n;return l==="chauffeur"?(console.log(`🌐 [CHAT] Chauffeur → Admin - Message: "${s.substring(0,50)}..."`),n!=="fr"?(o=await kt(s,"fr","auto",t.env.GOOGLE_TRANSLATE_API_KEY),console.log(`✅ [CHAT] Traduction FR: "${o.substring(0,50)}..."`)):console.log("ℹ️ [CHAT] Chauffeur français - pas de traduction nécessaire"),d=n,c=s):(console.log(`🌐 [CHAT] Admin → Chauffeur - Message: "${s.substring(0,50)}..."`),n!=="fr"?(c=await kt(s,n,"fr",t.env.GOOGLE_TRANSLATE_API_KEY),console.log(`✅ [CHAT] Traduction ${n}: "${c.substring(0,50)}..."`)):console.log("ℹ️ [CHAT] Chauffeur français - pas de traduction nécessaire"),d="fr",o=s),console.log(`💾 [CHAT] Insertion DB - message: "${s.substring(0,30)}...", translated_fr: "${o.substring(0,30)}...", translated_chauffeur: "${c.substring(0,30)}..."`),await t.env.DB.prepare(`
    ...
    `;try{const{results:s}=await t.env.DB.prepare(`
        ...
        WHERE ca.status = 'in_progress' 
        ORDER BY ca.arrival_time DESC
      `).all();return t.json({success:!0,chauffeurs:s})}catch{console.log("Table chauffeur_sessions not found, using simple query");const{results:i}=await t.env.DB.prepare(r).all();return t.json({success:!0,chauffeurs:i})}}catch(r){return console.error("Erreur liste chauffeurs:",r),t.json({success:!1,error:r.message},500)}});p.use("*",async(t,r)=>{const s=t.req.path;if(["/login","/test-questionnaire","/qrcode-chauffeur","/chauffeur/langue","/chauffeur/video","/chauffeur/inscription","/chauffeur/taches"].includes(s)||s.startsWith("/static/")||s.startsWith("/api/chauffeur/")){await r();return}await r()});p.use(Ei);p.get("/",t=>t.render(e(ki,{})));p.get("/reception",t=>t.render(e(Ti,{})));p.get("/accueil-chauffeur",t=>t.render(e(Ci,{})));p.get("/administrateur",t=>t.render(e(_i,{})));p.get("/controleur",t=>t.render(e(qi,{})));p.get("/agent-quai",t=>t.render(e(Ai,{})));p.get("/chef-equipe",t=>t.render(e(Fi,{})));p.get("/nouveau",t=>t.render(e(Si,{})));p.get("/archives",t=>t.render(e(ji,{})));p.get("/anomalies",t=>t.render(e(Ri,{})));p.get("/bibliotheque",t=>t.render(e(Ni,{})));p.get("/contacts",t=>t.render(e(Di,{})));p.get("/admin/chauffeurs-dashboard",t=>t.render(e(Bi,{})));p.post("/api/admin/chat",async t=>{try{const{chauffeur_id:r,message:s}=await t.req.json();if(!r||!s)return t.json({success:!1,error:"Données manquantes"},400);const i=await t.env.DB.prepare(`
      SELECT langue FROM chauffeur_arrivals WHERE id = ?
    `).bind(r).first(),a=(i==null?void 0:i.langue)||"fr";let n=s;a!=="fr"&&(n=await kt(s,a,"fr"));try{await t.env.DB.prepare(`
        INSERT INTO chat_messages (chauffeur_id, sender, message, original_lang, translated_fr, translated_chauffeur, read_by_admin, read_by_chauffeur)
        VALUES (?, 'admin', ?, 'fr', ?, ?, 1, 0)
