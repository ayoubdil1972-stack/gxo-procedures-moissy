...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(`✅ Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id} - Timer: voir historique`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme disponible (fallback) - Timer conservé dans commentaire")}let l=!1;try{console.log("🔍 Vérification alerte - Données reçues:",{palettes_attendues:r.palettes_attendues,palettes_recues:r.palettes_recues,problemes:r.problemes});const o=parseInt(r.palettes_attendues)!==parseInt(r.palettes_recues);console.log("📊 Écart palettes:",o,`(${r.palettes_attendues} vs ${r.palettes_recues})`);const c=r.problemes||[],d=c.length>0;console.log("⚠️ Non-conformités:",d,"Nombre:",c.length),console.log("📊 CRÉATION ALERTE KPI - Tous les camions sont enregistrés"),console.log("   Quai:",r.quai_numero),console.log("   ID:",r.numero_id),console.log("   Fournisseur:",r.fournisseur);{const u=new Date().toISOString().slice(0,19).replace("T"," ");console.log("⏰ Utilisation timer_start sauvegardé:",timerStartSauvegarde),console.log("⏰ timer_fin_timestamp (NOW):",u),await t.env.DB.prepare(`
        ...
        `).run(),console.log("✅ Table controleur_alertes vérifiée");let h="";o&&(h=`Écart palettes: Attendues ${r.palettes_attendues}, Reçues ${r.palettes_recues}`);const f=JSON.stringify(c);console.log("📝 Non-conformités (problèmes):",f);const g=JSON.stringify(r.verification_points||{});console.log("📝 Points de vérification:",g);const x=o||d?"en_attente":"traitee";console.log("📊 Statut alerte:",x,"(Problèmes:",o||d,")"),console.log("⏱️ Durée déchargement:",timerDuration,"secondes");const v=await t.env.DB.prepare(`
