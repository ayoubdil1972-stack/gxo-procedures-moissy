...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")).getTime(),o=new Date(Br()).getTime();n=Math.floor((o-l)/1e3),console.log(`⏱️ Durée contrôle calculée: ${n}s`)}return await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
...
</html>`));p.get("/chauffeur/consignes",t=>{const r=t.req.query("lang")||"fr";return t.redirect(`/consignes/${r}`)});p.get("/chauffeur/video",t=>t.redirect("/chauffeur/consignes?lang="+(t.req.query("lang")||"fr")));p.get("/chauffeur/inscription",t=>{const r=t.req.query("lang")||"fr";return t.html(Mi({lang:r}))});p.get("/chauffeur/taches",Ve,t=>t.render(e(Bi,{})));p.post("/api/chauffeur/inscription",async t=>{try{const{pseudo:r,entreprise:s,numero_quai:i,langue:a,video_completed:n}=await t.req.json(),l=await t.env.DB.prepare(`
    ...
    `).run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_chauffeur_id ON chat_messages(chauffeur_id)").run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)").run(),console.log("✅ Table chat_messages prête avec colonnes de traduction")}catch(r){console.error("⚠️ Erreur auto-migration chat_messages:",r.message)}}p.post("/api/chauffeur/chat",async t=>{try{await zr(t.env.DB);const{chauffeur_id:r,message:s,sender:i,lang:a}=await t.req.json();if(!r||!s)return t.json({success:!1,error:"Données manquantes"},400);let n=a||"fr";if(!a){const u=await t.env.DB.prepare(`
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();console.log("📊 Quai data:",o);let c=null;if(o!=null&&o.timer_start){const d=new Date(o.timer_start.replace(" ","T")).getTime(),u=new Date(Br()).getTime();c=Math.floor((u-d)/1e3),console.log(`⏱️ Durée calculée: ${c}s (${Math.floor(c/3600)}h ${Math.floor(c%3600/60)}m ${c%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:c,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
