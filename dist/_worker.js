...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")+"Z").getTime(),o=new Date(zr()).getTime();n=Math.floor((o-l)/1e3),console.log(`⏱️ Durée contrôle calculée: ${n}s`)}return await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_chauffeur_id ON chat_messages(chauffeur_id)").run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)").run(),console.log("✅ Table chat_messages prête avec colonnes de traduction")}catch(r){console.error("⚠️ Erreur auto-migration chat_messages:",r.message)}}p.post("/api/chauffeur/chat",async t=>{try{await Br(t.env.DB);const{chauffeur_id:r,message:s,sender:i,lang:a}=await t.req.json();if(!r||!s)return t.json({success:!1,error:"Données manquantes"},400);let n=a||"fr";if(!a){const m=await t.env.DB.prepare(`
    ...
    `).bind(r).run(),console.log(`✅ Chauffeur ${r} clôturé`),t.json({success:!0})}catch(r){return console.error("Erreur clôture chauffeur:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeur/notification",async t=>{try{const{chauffeur_id:r,type:s,titre:i,message:a}=await t.req.json();return await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(r,s,i,a).run(),t.json({success:!0})}catch(r){return console.error("Erreur création notification:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeurs/inscription",Ui);p.get("/api/chauffeurs/:id",Vi);p.get("/api/chauffeurs/:id/taches",Hi);p.post("/api/taches/:id/completer",Gi);p.get("/api/chauffeurs/:id/messages",Qi);p.post("/api/messages",Xi);p.post("/api/messages/:id/marquer-vu",Wi);p.get("/api/admin/chauffeurs-actifs",Zi);p.post("/api/admin/taches/assigner",Yi);p.get("/api/quais",async t=>{try{const{results:r}=await t.env.DB.prepare(`
