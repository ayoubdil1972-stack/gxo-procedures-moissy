...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).all();return console.log(`📚 Archives récupérées: ${((r=i.results)==null?void 0:r.length)||0}`),t.json({success:!0,count:((s=i.results)==null?void 0:s.length)||0,alertes:i.results||[]})}catch(i){return console.error("❌ Erreur récupération archives:",i),t.json({success:!1,error:i.message,count:null,alertes:[]})}});p.put("/api/controleur/alertes/:id",async t=>{try{const r=t.req.param("id"),s=await t.req.json();return console.log("📝 Traitement alerte:",r,s),!s.traite_par||!s.consignes?t.json({success:!1,error:"Contrôleur et consignes requis"},400):(await t.env.DB.prepare(`
      ...
      WHERE id = ?
    `).bind(s.consignes,s.traite_par,r).run(),console.log("✅ Alerte",r,"traitée par",s.traite_par),t.json({success:!0,message:"Alerte traitée avec succès"}))}catch(r){return console.error("❌ Erreur traitement alerte:",r),t.json({success:!1,error:r.message},500)}});p.get("/api/quais",async t=>{try{const{results:r}=await t.env.DB.prepare(`
