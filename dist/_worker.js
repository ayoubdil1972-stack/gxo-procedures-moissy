...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero).first();console.log("📊 Quai data AVANT UPDATE:",l);const o=l==null?void 0:l.timer_start;let c=null;if(l!=null&&l.timer_start){const u=new Date(l.timer_start.replace(" ","T")).getTime(),h=new Date(Hr()).getTime();c=Math.floor((h-u)/1e3),console.log(`⏱️ Durée calculée: ${c}s (${Math.floor(c/3600)}h ${Math.floor(c%3600/60)}m ${c%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:c,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
      UPDATE quai_status 
