...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function Ui(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a||null).run();return t.json({success:!0,id:n.meta.last_row_id,message:"Tâche assignée"})}catch(r){return console.error("Erreur assignation tâche:",r),t.json({error:"Erreur serveur"},500)}}const p=new Rr;p.use("/static/*",Ws({root:"./",onNotFound:(t,r)=>(console.log("Fichier non trouvé:",t),r.notFound())}));p.get("/login",Ve,t=>t.render(e(Di,{})));p.get("/qrcode-chauffeur",Ve,t=>t.render(e(ji,{})));p.get("/chauffeur/langue",Ve,t=>t.render(e(Pi,{})));p.get("/gestion-quais",Ve,t=>t.render(e(Ii,{})));p.get("/pdf-barcodes",t=>t.html(zi()));p.get("/qrcode-generator",t=>t.html(Bi()));p.get("/scan",t=>{const r=t.req.query("quai"),s=t.req.query("action")||"start";return r?t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
      ...
      <script>
        ...
        document.getElementById('finControleForm').addEventListener('submit', async (e) => {
          ...
          try {
            ...
            window.location.href = '/scan-fin-controle-success?quai=${r}&duree=' + result.dureeControle + '&nom=' + encodeURIComponent(controleurNom);
          } catch (error) {
            console.error('Erreur:', error);
  ...
  `):t.html(`
    ...
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")+"Z").getTime(),o=Date.now();n=Math.floor((o-l)/1e3),console.log(`⏱️ Durée contrôle calculée: ${n}s`)}return await t.env.DB.prepare(`
      ...
      SET statut = 'fin_controle',
          ...
          updated_at = datetime('now')
      WHERE quai_numero = ?
    `).bind(n,i,s).run(),console.log(`✅ Quai ${s} passé en fin de contrôle - Timer figé à ${n}s - Contrôleur: ${i}`),t.json({success:!0,quai:s,dureeControle:n,controleurNom:i})}catch(r){return console.error("❌ Erreur fin contrôle:",r),t.json({error:"Erreur serveur"},500)}});p.get("/scan-fin-controle-success",async t=>{const r=t.req.query("quai"),s=parseInt(t.req.query("duree")||"0"),i=t.req.query("nom")||"",a=Math.floor(s/3600),n=Math.floor(s%3600/60),l=s%60,o=`${String(a).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(l).padStart(2,"0")}`;return t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
...
</html>`));p.get("/chauffeur/consignes",t=>{const r=t.req.query("lang")||"fr";return t.redirect(`/consignes/${r}`)});p.get("/chauffeur/video",t=>t.redirect("/chauffeur/consignes?lang="+(t.req.query("lang")||"fr")));p.get("/chauffeur/inscription",t=>{const r=t.req.query("lang")||"fr";return t.html(Oi({lang:r}))});p.get("/chauffeur/taches",Ve,t=>t.render(e(Mi,{})));p.post("/api/chauffeur/inscription",async t=>{try{const{pseudo:r,entreprise:s,numero_quai:i,langue:a,video_completed:n}=await t.req.json(),l=await t.env.DB.prepare(`
    ...
    `).bind(r).run(),console.log(`✅ Chauffeur ${r} clôturé`),t.json({success:!0})}catch(r){return console.error("Erreur clôture chauffeur:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeur/notification",async t=>{try{const{chauffeur_id:r,type:s,titre:i,message:a}=await t.req.json();return await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(r,s,i,a).run(),t.json({success:!0})}catch(r){return console.error("Erreur création notification:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeurs/inscription",Ui);p.get("/api/chauffeurs/:id",$i);p.get("/api/chauffeurs/:id/taches",Vi);p.post("/api/taches/:id/completer",Hi);p.get("/api/chauffeurs/:id/messages",Gi);p.post("/api/messages",Qi);p.post("/api/messages/:id/marquer-vu",Xi);p.get("/api/admin/chauffeurs-actifs",Wi);p.post("/api/admin/taches/assigner",Yi);p.get("/api/quais",async t=>{try{const{results:r}=await t.env.DB.prepare(`
