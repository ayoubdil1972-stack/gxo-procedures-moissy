...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
...
</html>`));p.get("/chauffeur/consignes",t=>{const r=t.req.query("lang")||"fr";return t.redirect(`/consignes/${r}`)});p.get("/chauffeur/video",t=>t.redirect("/chauffeur/consignes?lang="+(t.req.query("lang")||"fr")));p.get("/chauffeur/inscription",t=>{const r=t.req.query("lang")||"fr";return t.html(Mi({lang:r}))});p.get("/chauffeur/taches",$e,t=>t.render(e(Hi,{})));p.post("/api/chauffeur/inscription",async t=>{try{const{pseudo:r,entreprise:s,numero_quai:i,langue:a,video_completed:n}=await t.req.json(),l=await t.env.DB.prepare(`
    ...
    `).bind(i||null,r).run(),console.log(`✅ Improductivité ${r} validée`),t.json({success:!0,message:"Improductivité validée et transmise"})}catch(r){return console.error("❌ Erreur validation improductivité:",r),t.json({success:!1,error:r.message},500)}});p.get("/api/improductivites/utilisateur/:nom",async t=>{try{const r=t.req.param("nom");if(!r)return t.json({success:!1,error:"Nom requis"},400);const{results:s}=await t.env.DB.prepare(`
      ...
      ORDER BY created_at DESC
      LIMIT 100
    `).bind(r).all();return t.json({success:!0,improductivites:s})}catch(r){return console.error("❌ Erreur récupération improductivités utilisateur:",r),t.json({success:!0,improductivites:[]})}});p.get("/api/chef-equipe/kpi/reception-camion",async t=>{try{const r=t.req.query("date");let s=`
