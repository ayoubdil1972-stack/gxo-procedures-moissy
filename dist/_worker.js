...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).run();let s="SELECT * FROM kpi_archives",i=[];r?(s+=" WHERE DATE(created_at) = ?",i.push(r)):s+=' WHERE DATE(created_at) = DATE("now")',s+=" ORDER BY created_at DESC LIMIT 200";const{results:a}=await t.env.DB.prepare(s).bind(...i).all();console.log(`📊✅ KPI: ${a.length} quais archivés pour ${r||"aujourd'hui"}`);const n=a.map(c=>{const d=g=>{if(!g)return"—";const x=Math.floor(g/3600),v=Math.floor(g%3600/60),b=g%60;return`${String(x).padStart(2,"0")}:${String(v).padStart(2,"0")}:${String(b).padStart(2,"0")}`},u=(g,x,v)=>{if(!g)return"grey";const b=Math.round(g/60);return b<=x?"green":b<=v?"orange":"red"},h=c.duree_dechargement_secondes||0,f=c.duree_controle_secondes||0;return{id:c.id,quai_numero:c.quai_numero,numero_camion:c.numero_camion,fournisseur:c.fournisseur,duree_dechargement:d(h),duree_dechargement_minutes:Math.round(h/60),duree_dechargement_status:u(h,20,25),duree_controle:d(f),duree_controle_minutes:Math.round(f/60),duree_controle_status:u(f,30,40),controleur_nom:c.controleur_nom||"—",created_at:c.created_at}}),l=n.length,o={total_camions:l,dechargement_minutes:l>0?Math.round(n.reduce((c,d)=>c+d.duree_dechargement_minutes,0)/l):0,controle_minutes:l>0?Math.round(n.reduce((c,d)=>c+d.duree_controle_minutes,0)/l):0};return console.log("📊✅ Moyennes:",o),t.json({success:!0,kpi:n,moyennes:o,date:r||new Date().toISOString().split("T")[0]})}catch(r){return console.error("❌ Erreur chargement KPI:",r),t.json({success:!1,error:r.message},500)}});const Zt=new Sr,ea=Object.assign({"/src/index.tsx":p});let Ur=!1;for(const[,t]of Object.entries(ea))t&&(Zt.route("/",t),Zt.notFound(t.notFoundHandler),Ur=!0);if(!Ur)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{Zt as default};
