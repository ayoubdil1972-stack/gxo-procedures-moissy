...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")).getTime(),o=new Date(Hr()).getTime();n=Math.floor((o-l)/1e3),console.log(`⏱️ Durée contrôle calculée (JavaScript): ${n}s`)}await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero).first();console.log("📊 Quai data AVANT UPDATE:",l);const o=l==null?void 0:l.timer_start;let c=null;if(l!=null&&l.timer_start){const u=new Date(l.timer_start.replace(" ","T")).getTime(),h=new Date(Hr()).getTime();c=Math.floor((h-u)/1e3),console.log(`⏱️ Durée calculée (JavaScript): ${c}s (${Math.floor(c/3600)}h ${Math.floor(c%3600/60)}m ${c%60}s)`)}console.log("📊 Timer start:",l==null?void 0:l.timer_start),console.log("💾 UPDATE avec:",{timerDuration:c,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
      UPDATE quai_status 
