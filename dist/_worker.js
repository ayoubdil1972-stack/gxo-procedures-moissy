...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();console.log("📊 Quai data AVANT UPDATE:",o);const c=o==null?void 0:o.timer_start;let d=null;if(o!=null&&o.timer_start){const u=await t.env.DB.prepare(`
          SELECT 
            CAST((julianday('now') - julianday(?)) * 86400 AS INTEGER) as raw_duration
        `).bind(o.timer_start).first(),h=(u==null?void 0:u.raw_duration)||0;d=h>=3600?h-3600:h,console.log(`⏱️ DÉCHARGEMENT: timer_start=${o.timer_start}, Brut=${h}s, Corrigé=${d}s (${Math.floor(d/60)}min ${d%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:d,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
      ...
      `).bind(`✅ Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id} - Timer: voir historique`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme disponible (fallback) - Timer conservé dans commentaire")}let l=!1;try{console.log("🔍 Vérification alerte - Données reçues:",{palettes_attendues:r.palettes_attendues,palettes_recues:r.palettes_recues,problemes:r.problemes});const o=parseInt(r.palettes_attendues)!==parseInt(r.palettes_recues);console.log("📊 Écart palettes:",o,`(${r.palettes_attendues} vs ${r.palettes_recues})`);const c=r.problemes||[],d=c.length>0;console.log("⚠️ Problèmes cochés:",d,"Nombre:",c.length);const u=r.verification_points||{},h=Object.values(u).filter(x=>x==="non_conforme"),f=h.length>0;console.log("❌ Points non conformes:",f,"Nombre:",h.length);const g=d||f;console.log("🚨 Nécessite attention contrôleur:",g),console.log("📊 CRÉATION ALERTE KPI - Tous les camions sont enregistrés"),console.log("   Quai:",r.quai_numero),console.log("   ID:",r.numero_id),console.log("   Fournisseur:",r.fournisseur);{const x=new Date().toISOString().slice(0,19).replace("T"," ");console.log("⏰ Utilisation timer_start sauvegardé:",timerStartSauvegarde),console.log("⏰ timer_fin_timestamp (NOW):",x),await t.env.DB.prepare(`
        ...
        `).run(),console.log("✅ Table controleur_alertes vérifiée");let v="";o&&(v=`Écart palettes: Attendues ${r.palettes_attendues}, Reçues ${r.palettes_recues}`);const b=JSON.stringify(c);console.log("📝 Non-conformités (problèmes):",b);const E=JSON.stringify(r.verification_points||{});console.log("📝 Points de vérification:",E);const T=o||g?"en_attente":"traitee";console.log("📊 Statut alerte:",T,"(Problèmes:",o||g,")"),console.log("⏱️ Durée déchargement:",timerDuration,"secondes");const C=await t.env.DB.prepare(`
