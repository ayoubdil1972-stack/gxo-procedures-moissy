...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function $i(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r).all();return t.json({messages:s.results||[]})}catch(r){return console.error("Erreur récupération messages:",r),t.json({error:"Erreur serveur"},500)}}async function Wi(t){try{const{chauffeur_id:r,message:s,expediteur:i}=await t.req.json();if(!r||!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a||null).run();return t.json({success:!0,id:n.meta.last_row_id,message:"Tâche assignée"})}catch(r){return console.error("Erreur assignation tâche:",r),t.json({error:"Erreur serveur"},500)}}const p=new Sr;p.use("/static/*",Ws({root:"./",onNotFound:(t,r)=>(console.log("Fichier non trouvé:",t),r.notFound())}));p.get("/login",$e,t=>t.render(e(Fi,{})));p.get("/qrcode-chauffeur",$e,t=>t.render(e(Ii,{})));p.get("/chauffeur/langue",$e,t=>t.render(e(Li,{})));p.get("/gestion-quais",$e,t=>t.render(e(Bi,{})));p.get("/pdf-barcodes",t=>t.html(Ui()));p.get("/qrcode-generator",t=>t.html(zi()));p.get("/scan-dechargement",t=>{const r=t.req.query("quai");return t.redirect(`/scan?quai=${r}`)});p.get("/scan",t=>{const r=t.req.query("quai"),s=t.req.query("action")||"start";return r?t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first(),n=(a==null?void 0:a.duration_seconds)||0;console.log(`⏱️ Durée contrôle calculée (SQLite): ${n}s`),console.log("📊 Timer contrôle start:",a==null?void 0:a.timer_controle_start),await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r).run(),console.log(`✅ Chauffeur ${r} clôturé`),t.json({success:!0})}catch(r){return console.error("Erreur clôture chauffeur:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeur/notification",async t=>{try{const{chauffeur_id:r,type:s,titre:i,message:a}=await t.req.json();return await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(r,s,i,a).run(),t.json({success:!0})}catch(r){return console.error("Erreur création notification:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeurs/inscription",$i);p.get("/api/chauffeurs/:id",Vi);p.get("/api/chauffeurs/:id/taches",Gi);p.post("/api/taches/:id/completer",Qi);p.get("/api/chauffeurs/:id/messages",Xi);p.post("/api/messages",Wi);p.post("/api/messages/:id/marquer-vu",Yi);p.get("/api/admin/chauffeurs-actifs",Zi);p.post("/api/admin/taches/assigner",Ji);p.post("/api/controleur/improd",async t=>{try{const r=await t.req.json();if(console.log("📋 Improductivité reçue:",r),!r.controleur_nom||!r.raison||!r.duree)return t.json({success:!1,error:"Données manquantes"},400);await t.env.DB.prepare(`
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);const l=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(r.quai_numero).first(),o=(l==null?void 0:l.duration_seconds)||0;console.log(`⏱️ Durée calculée (SQLite): ${o}s (${Math.floor(o/3600)}h ${Math.floor(o%3600/60)}m ${o%60}s)`),console.log("📊 Timer start:",l==null?void 0:l.timer_start),console.log("💾 UPDATE avec:",{timerDuration:o,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
      UPDATE quai_status 
