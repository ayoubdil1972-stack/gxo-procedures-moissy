...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `,i=[];r?(s+=" AND DATE(archived_at) = ?",i.push(r)):s+=' AND DATE(archived_at) = DATE("now")',s+=" ORDER BY archived_at DESC LIMIT 200";const{results:a}=await t.env.DB.prepare(s).bind(...i).all();console.log(`📊✅ KPI: ${a.length} quais archivés pour ${r||"aujourd'hui"}`);const n=a.map(u=>({...u,quai_numero:u.quai_numero})),l=n.length;let o=0,c=0;n.forEach(u=>{u.timer_duration&&(o+=u.timer_duration),u.timer_controle_duration&&(c+=u.timer_controle_duration)});const d={total_camions:l,dechargement_minutes:l>0?Math.round(o/l/60):0,controle_minutes:l>0?Math.round(c/l/60):0};return console.log("📊✅ Moyennes:",d),t.json({success:!0,quais:n,moyennes:d,date:r||new Date().toISOString().split("T")[0]})}catch(r){return console.error("❌ Erreur chargement KPI:",r),t.json({success:!1,error:r.message},500)}});const Zt=new Sr,ea=Object.assign({"/src/index.tsx":p});let Br=!1;for(const[,t]of Object.entries(ea))t&&(Zt.route("/",t),Zt.notFound(t.notFoundHandler),Br=!0);if(!Br)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{Zt as default};
