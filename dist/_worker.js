...
<body>
    ...
    `;try{const{results:i}=await t.env.DB.prepare(`
        ...
        WHERE ca.status = 'in_progress' 
        ORDER BY ca.arrival_time DESC
      `).all();return t.json({success:!0,chauffeurs:i})}catch{console.log("Table chauffeur_sessions not found, using simple query");const{results:s}=await t.env.DB.prepare(r).all();return t.json({success:!0,chauffeurs:s})}}catch(r){return console.error("Erreur liste chauffeurs:",r),t.json({success:!1,error:r.message},500)}});v.use("*",async(t,r)=>{const i=t.req.path;if(["/login","/test-questionnaire","/qrcode-chauffeur","/chauffeur/langue","/chauffeur/video","/chauffeur/inscription","/chauffeur/taches"].includes(i)||i.startsWith("/static/")||i.startsWith("/api/chauffeur/")){await r();return}await r()});v.use(ws);v.get("/",t=>t.render(e(ks,{})));v.get("/reception",t=>t.render(e(Es,{})));v.get("/accueil-chauffeur",t=>t.render(e(Cs,{})));v.get("/administrateur",t=>t.render(e(Ts,{})));v.get("/controleur",t=>t.render(e(As,{})));v.get("/agent-quai",t=>t.render(e(Ss,{})));v.get("/nouveau",t=>t.render(e(Rs,{})));v.get("/anomalies",t=>t.render(e(qs,{})));v.get("/bibliotheque",t=>t.render(e(_s,{})));v.get("/contacts",t=>t.render(e(js,{})));v.get("/admin/chauffeurs-dashboard",t=>t.render(e(Ls,{})));v.post("/api/admin/chat",async t=>{try{const{chauffeur_id:r,message:i}=await t.req.json();if(!r||!i)return t.json({success:!1,error:"Données manquantes"},400);const s=await t.env.DB.prepare(`
      SELECT langue FROM chauffeur_arrivals WHERE id = ?
