...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")).getTime(),o=new Date(Hr()).getTime();let c=Math.floor((o-l)/1e3);n=Math.max(0,c-3600),console.log(`⏱️ Durée contrôle calculée: ${c}s → Corrigée (-1h): ${n}s`)}await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();console.log("📊 Quai data AVANT UPDATE:",o);const c=o==null?void 0:o.timer_start;let d=null;if(o!=null&&o.timer_start){const u=new Date(o.timer_start.replace(" ","T")).getTime(),h=new Date(Hr()).getTime();let f=Math.floor((h-u)/1e3);d=Math.max(0,f-3600),console.log(`⏱️ Durée calculée: ${f}s → Corrigée (-1h): ${d}s`)}console.log("💾 UPDATE avec:",{timerDuration:d,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
