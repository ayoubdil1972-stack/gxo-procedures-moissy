...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.controleur_nom,r.raison,r.duree,r.commentaire||"",r.date_debut,r.date_fin).run();return console.log("✅ Improductivité enregistrée:",s.meta.last_row_id),t.json({success:!0,id:s.meta.last_row_id})}catch(r){return console.error("❌ Erreur enregistrement improductivité:",r),t.json({success:!1,error:r.message},500)}});p.get("/api/controleur/improd/historique",async t=>{try{const r=new Date().toISOString().split("T")[0],{results:s}=await t.env.DB.prepare(`
      ...
      LIMIT 50
    `).all();return t.json({success:!0,improds:s||[]})}catch(r){return console.error("❌ Erreur récupération historique:",r),t.json({success:!0,improds:[]})}});p.get("/api/quais",async t=>{try{const{results:r}=await t.env.DB.prepare(`
