    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(c,`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme fin de déchargement - Timer figé à",c,"secondes (timer_start supprimé)")}catch(o){console.warn("⚠️ Contrainte CHECK - Fallback vers disponible:",o.message),await t.env.DB.prepare(`
        ...
        SET statut = 'disponible',
            ...
            updated_at = datetime('now', 'localtime')
        WHERE quai_numero = ?
      `).bind(`✅ Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id} - Timer: voir historique`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme disponible (fallback) - Timer conservé dans commentaire")}let l=!1;try{const o=parseInt(r.palettes_attendues)!==parseInt(r.palettes_recues),c=r.problemes||[],d=c.length>0;if(o||d){console.log("🚨 Écart ou non-conformité détecté - Création alerte contrôleur");const u=await t.env.DB.prepare(`
          SELECT timer_start, timer_fin_timestamp FROM quai_status WHERE quai_numero = ?
        `).bind(r.quai_numero).first();await t.env.DB.prepare(`
        ...
        `).run();let h="";o&&(h=`Écart palettes: Attendues ${r.palettes_attendues}, Reçues ${r.palettes_recues}`);const f=JSON.stringify(c),g=await t.env.DB.prepare(`
