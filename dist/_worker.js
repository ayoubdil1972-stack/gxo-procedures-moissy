...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function $i(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a||null).run();return t.json({success:!0,id:n.meta.last_row_id,message:"Tâche assignée"})}catch(r){return console.error("Erreur assignation tâche:",r),t.json({error:"Erreur serveur"},500)}}const p=new Nr;p.use("/static/*",Xs({root:"./",onNotFound:(t,r)=>(console.log("Fichier non trouvé:",t),r.notFound())}));p.get("/login",Ve,t=>t.render(e(Fi,{})));p.get("/qrcode-chauffeur",Ve,t=>t.render(e(Oi,{})));p.get("/chauffeur/langue",Ve,t=>t.render(e(Ii,{})));p.get("/gestion-quais",Ve,t=>t.render(e(Ui,{})));p.get("/pdf-barcodes",t=>t.html(Bi()));p.get("/qrcode-generator",t=>t.html(zi()));p.get("/scan-dechargement",t=>{const r=t.req.query("quai");return t.redirect(`/scan?quai=${r}`)});p.get("/scan",t=>{const r=t.req.query("quai"),s=t.req.query("action")||"start";return r?t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);let l=null,o=null;try{const u=await t.env.DB.prepare(`
      ...
      `).bind(`✅ Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id} - Timer: voir historique`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme disponible (fallback) - Timer conservé dans commentaire")}let c=!1,d=null;try{console.log("🔍 Vérification alerte - Données reçues:",{palettes_attendues:r.palettes_attendues,palettes_recues:r.palettes_recues,problemes:r.problemes});const u=parseInt(r.palettes_attendues)!==parseInt(r.palettes_recues);console.log("📊 Écart palettes:",u,`(${r.palettes_attendues} vs ${r.palettes_recues})`);const h=r.problemes||[],f=h.length>0;console.log("⚠️ Problèmes cochés:",f,"Nombre:",h.length);const b=r.verification_points||{},x=Object.values(b).filter(w=>w==="non_conforme"),v=x.length>0;console.log("❌ Points non conformes:",v,"Nombre:",x.length);const g=f||v;console.log("🚨 Nécessite attention contrôleur:",g),console.log("📊 CRÉATION ALERTE KPI - Tous les camions sont enregistrés"),console.log("   Quai:",r.quai_numero),console.log("   ID:",r.numero_id),console.log("   Fournisseur:",r.fournisseur);{const w=new Date().toISOString().slice(0,19).replace("T"," ");console.log("⏰ Utilisation timer_start sauvegardé:",l),console.log("⏰ timer_fin_timestamp (NOW):",w),await t.env.DB.prepare(`
        ...
        `).run(),console.log("✅ Table controleur_alertes vérifiée");let T="";u&&(T=`Écart palettes: Attendues ${r.palettes_attendues}, Reçues ${r.palettes_recues}`);const C=JSON.stringify(h);console.log("📝 Non-conformités (problèmes):",C);const k=JSON.stringify(r.verification_points||{});console.log("📝 Points de vérification:",k);const A=u||g?"en_attente":"traitee";console.log("📊 Statut alerte:",A,"(Problèmes:",u||g,")"),console.log("⏱️ Durée déchargement:",o,"secondes");const re=await t.env.DB.prepare(`
