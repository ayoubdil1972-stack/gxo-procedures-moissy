...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function Vi(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a||null).run();return t.json({success:!0,id:n.meta.last_row_id,message:"Tâche assignée"})}catch(r){return console.error("Erreur assignation tâche:",r),t.json({error:"Erreur serveur"},500)}}const p=new Sr;function Hr(){const t=new Date;return new Date(t.toLocaleString("en-US",{timeZone:"Europe/Paris"})).toISOString()}p.use("/static/*",Ys({root:"./",onNotFound:(t,r)=>(console.log("Fichier non trouvé:",t),r.notFound())}));p.get("/login",$e,t=>t.render(e(Pi,{})));p.get("/qrcode-chauffeur",$e,t=>t.render(e(Oi,{})));p.get("/chauffeur/langue",$e,t=>t.render(e(Ii,{})));p.get("/gestion-quais",$e,t=>t.render(e(zi,{})));p.get("/pdf-barcodes",t=>t.html(Ui()));p.get("/qrcode-generator",t=>t.html($i()));p.get("/scan-dechargement",t=>{const r=t.req.query("quai");return t.redirect(`/scan?quai=${r}`)});p.get("/scan",t=>{const r=t.req.query("quai"),s=t.req.query("action")||"start";return r?t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();console.log("📊 Quai data:",o);let c=null;if(o!=null&&o.timer_start){const d=new Date(o.timer_start.replace(" ","T")).getTime(),u=new Date(Hr()).getTime();c=Math.floor((u-d)/1e3),console.log(`⏱️ Durée calculée: ${c}s (${Math.floor(c/3600)}h ${Math.floor(c%3600/60)}m ${c%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:c,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
        SET statut = 'fin_dechargement',
            ...
            updated_at = datetime('now', 'localtime')
        WHERE quai_numero = ?
      `).bind(c,`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme fin de déchargement - Timer figé à",c,"secondes (timer_start supprimé)")}catch(o){console.warn("⚠️ Contrainte CHECK - Fallback vers disponible:",o.message),await t.env.DB.prepare(`
        UPDATE quai_status 
      ...
      `).bind(`✅ Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id} - Timer: voir historique`,r.nom_agent,r.quai_numero).run(),console.log("✅ Quai",r.quai_numero,"marqué comme disponible (fallback) - Timer conservé dans commentaire")}let l=!1;try{console.log("🔍 Vérification alerte - Données reçues:",{palettes_attendues:r.palettes_attendues,palettes_recues:r.palettes_recues,problemes:r.problemes});const o=parseInt(r.palettes_attendues)!==parseInt(r.palettes_recues);console.log("📊 Écart palettes:",o,`(${r.palettes_attendues} vs ${r.palettes_recues})`);const c=r.problemes||[],d=c.length>0;if(console.log("⚠️ Non-conformités:",d,"Nombre:",c.length),o||d){console.log("🚨 DÉCLENCHEMENT ALERTE - Création en cours..."),console.log("   Quai:",r.quai_numero),console.log("   ID:",r.numero_id),console.log("   Fournisseur:",r.fournisseur);const u=await t.env.DB.prepare(`
        ...
        `).run(),console.log("✅ Table controleur_alertes vérifiée");let h="";o&&(h=`Écart palettes: Attendues ${r.palettes_attendues}, Reçues ${r.palettes_recues}`);const f=JSON.stringify(c);console.log("📝 Non-conformités (problèmes):",f);const g=JSON.stringify(r.verification_points||{});console.log("📝 Points de vérification:",g);const x=await t.env.DB.prepare(`
    ...
    `).bind(i||null,r).run(),console.log(`✅ Improductivité ${r} validée`),t.json({success:!0,message:"Improductivité validée et transmise"})}catch(r){return console.error("❌ Erreur validation improductivité:",r),t.json({success:!1,error:r.message},500)}});p.get("/api/improductivites/utilisateur/:nom",async t=>{try{const r=t.req.param("nom");if(!r)return t.json({success:!1,error:"Nom requis"},400);const{results:s}=await t.env.DB.prepare(`
      ...
      ORDER BY created_at DESC
      LIMIT 100
    `).bind(r).all();return t.json({success:!0,improductivites:s})}catch(r){return console.error("❌ Erreur récupération improductivités utilisateur:",r),t.json({success:!0,improductivites:[]})}});p.get("/api/chef-equipe/kpi/reception-camion",async t=>{try{const s=t.req.query("date")||new Date().toISOString().split("T")[0],{results:i}=await t.env.DB.prepare(`
      SELECT 
        id,
        quai_numero,
      ...
      ORDER BY created_at DESC
      LIMIT 100
    `).bind(s).all(),a=i.map(o=>{const c=o.heure_debut_dechargement?new Date(o.heure_debut_dechargement):null,d=o.heure_fin_dechargement?new Date(o.heure_fin_dechargement):null;let u=null,h=null;c&&d&&(u=Math.round((d.getTime()-c.getTime())/6e4),u<=20?h="vert":u<=25?h="orange":h="rouge");const f=o.heure_validation_controle?new Date(o.heure_validation_controle):null;let g=null,x=null;d&&f&&(g=Math.round((f.getTime()-d.getTime())/6e4),g<=30?x="vert":g<=40?x="orange":x="rouge");let v=null,b=null;return c&&f&&(v=Math.round((f.getTime()-c.getTime())/6e4),v<=60?b="vert":v<=70?b="orange":b="rouge"),{id:o.id,quai_numero:o.quai_numero,numero_camion:o.numero_camion||`CAM-${o.id}`,fournisseur:o.fournisseur,heure_debut_dechargement:o.heure_debut_dechargement,heure_fin_dechargement:o.heure_fin_dechargement,heure_validation_controle:o.heure_validation_controle,temps_dechargement_minutes:u,temps_dechargement_statut:h,temps_controle_minutes:g,temps_controle_statut:x,temps_total_minutes:v,temps_total_statut:b}}),n=a.filter(o=>o.temps_dechargement_minutes!==null&&o.temps_controle_minutes!==null&&o.temps_total_minutes!==null),l={temps_dechargement_moyen:n.length>0?Math.round(n.reduce((o,c)=>o+c.temps_dechargement_minutes,0)/n.length):0,temps_controle_moyen:n.length>0?Math.round(n.reduce((o,c)=>o+c.temps_controle_minutes,0)/n.length):0,temps_total_moyen:n.length>0?Math.round(n.reduce((o,c)=>o+c.temps_total_minutes,0)/n.length):0,nombre_camions:a.length,nombre_camions_complets:n.length};return t.json({success:!0,date:s,kpi:a,moyennes:l})}catch(r){return console.error("❌ Erreur récupération KPI réception:",r),t.json({success:!1,error:r.message},500)}});const Zt=new Sr,ea=Object.assign({"/src/index.tsx":p});let zr=!1;for(const[,t]of Object.entries(ea))t&&(Zt.route("/",t),Zt.notFound(t.notFoundHandler),zr=!0);if(!zr)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{Zt as default};
