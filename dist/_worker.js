...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `):t.html(`
    ...
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=await t.env.DB.prepare(`
      ...
      `).bind(a.timer_controle_start).first();n=(l==null?void 0:l.duration)||0,console.log(`⏱️ CONTRÔLE: Durée = ${n}s (${Math.floor(n/60)}min ${n%60}s)`)}await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();console.log("📊 Quai data AVANT UPDATE:",o);const c=o==null?void 0:o.timer_start;let d=null;if(o!=null&&o.timer_start){const u=await t.env.DB.prepare(`
          SELECT unixepoch('now', 'localtime') - unixepoch(?) as duration
        `).bind(o.timer_start).first();d=(u==null?void 0:u.duration)||0,console.log(`⏱️ DÉCHARGEMENT: Durée = ${d}s (${Math.floor(d/60)}min ${d%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:d,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
