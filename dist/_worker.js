...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function $i(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a,n,l||"fr").run()).meta.last_row_id;return t.json({success:!0,id:c,message:"Inscription réussie"})}catch(r){return console.error("Erreur inscription chauffeur:",r),t.json({error:"Erreur lors de l'inscription"},500)}}async function Vi(t){try{const r=t.req.param("id"),s=await t.env.DB.prepare(`
      SELECT id, nom, entreprise, telephone, plaque, type_camion, langue, statut, date_arrivee
      FROM chauffeurs_v2
      WHERE id = ?
    `).bind(r).first();return s?t.json(s):t.json({error:"Chauffeur non trouvé"},404)}catch(r){return console.error("Erreur récupération chauffeur:",r),t.json({error:"Erreur serveur"},500)}}async function Gi(t){try{const r=t.req.param("id"),s=await t.env.DB.prepare(`
    ...
    `).bind(r).all();return t.json({taches_en_cours:s.results||[],taches_terminees:i.results||[]})}catch(r){return console.error("Erreur récupération tâches:",r),t.json({error:"Erreur serveur"},500)}}async function Qi(t){try{const r=t.req.param("id");return await t.env.DB.prepare(`
      ...
      WHERE id = ?
    `).bind(r).run(),t.json({success:!0,message:"Tâche terminée"})}catch(r){return console.error("Erreur complétion tâche:",r),t.json({error:"Erreur serveur"},500)}}async function Wi(t){try{const r=t.req.param("id"),s=await t.env.DB.prepare(`
      SELECT id, expediteur, message, date_envoi, vu
      FROM messages_v2
      WHERE chauffeur_id = ?
      ORDER BY date_envoi ASC
    `).bind(r).all();return t.json({messages:s.results||[]})}catch(r){return console.error("Erreur récupération messages:",r),t.json({error:"Erreur serveur"},500)}}async function Xi(t){try{const{chauffeur_id:r,message:s,expediteur:i}=await t.req.json();if(!r||!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      INSERT INTO messages_v2 (chauffeur_id, expediteur, message, date_envoi, vu)
      VALUES (?, ?, ?, CURRENT_TIMESTAMP, 0)
    `).bind(r,i,s).run();return t.json({success:!0,id:a.meta.last_row_id,message:"Message envoyé"})}catch(r){return console.error("Erreur envoi message:",r),t.json({error:"Erreur serveur"},500)}}async function Yi(t){try{const r=t.req.param("id");return await t.env.DB.prepare(`
      UPDATE messages_v2
      SET vu = 1
      WHERE id = ?
    `).bind(r).run(),t.json({success:!0})}catch(r){return console.error("Erreur marquage message:",r),t.json({error:"Erreur serveur"},500)}}async function Zi(t){try{const r=await t.env.DB.prepare(`
      SELECT 
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first();let n=null;if(a!=null&&a.controle_debut_timestamp){const l=await t.env.DB.prepare(`
        SELECT 
          CAST((julianday('now') - julianday(?)) * 86400 AS INTEGER) as raw_duration
      `).bind(a.controle_debut_timestamp).first(),o=(l==null?void 0:l.raw_duration)||0;n=o>=3600?o-3600:o,console.log(`⏱️ CONTRÔLE: debut=${a.controle_debut_timestamp}, Brut=${o}s, Corrigé=${n}s (${Math.floor(n/60)}min ${n%60}s)`)}await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
...
</html>`));p.get("/chauffeur/consignes",t=>{const r=t.req.query("lang")||"fr";return t.redirect(`/consignes/${r}`)});p.get("/chauffeur/video",t=>t.redirect("/chauffeur/consignes?lang="+(t.req.query("lang")||"fr")));p.get("/chauffeur/inscription",t=>{const r=t.req.query("lang")||"fr";return t.html(Pi({lang:r}))});p.get("/chauffeur/taches",Ve,t=>t.render(e(Mi,{})));p.post("/api/chauffeur/inscription",async t=>{try{const{pseudo:r,entreprise:s,numero_quai:i,langue:a,video_completed:n}=await t.req.json(),l=await t.env.DB.prepare(`
    ...
    `).bind(r).first();return t.json({success:!0,...s})}catch(r){return console.error("Erreur progression:",r),t.json({success:!1,error:r.message},500)}});async function Ur(t){try{await t.prepare(`
    ...
    `).run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_chauffeur_id ON chat_messages(chauffeur_id)").run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)").run(),console.log("✅ Table chat_messages prête avec colonnes de traduction")}catch(r){console.error("⚠️ Erreur auto-migration chat_messages:",r.message)}}p.post("/api/chauffeur/chat",async t=>{try{await Ur(t.env.DB);const{chauffeur_id:r,message:s,sender:i,lang:a}=await t.req.json();if(!r||!s)return t.json({success:!1,error:"Données manquantes"},400);let n=a||"fr";if(!a){const u=await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(r,l,s,d,o,c,l==="chauffeur"?0:1,l==="admin"?0:1).run(),console.log(`✅ [CHAT] Message enregistré avec succès - ID chauffeur: ${r}`),t.json({success:!0,translated_fr:o,translated_chauffeur:c})}catch(r){return console.error("Erreur envoi message:",r),t.json({success:!1,error:r.message},500)}});p.get("/api/chauffeur/chat",async t=>{try{await Ur(t.env.DB);const r=t.req.query("id")||t.req.query("chauffeur_id"),s=t.req.query("viewer")||"chauffeur";console.log(`📥 [CHAT GET] Récupération messages - Chauffeur: ${r}, Viewer: ${s}`);const i=await t.env.DB.prepare(`
      SELECT langue FROM chauffeur_arrivals WHERE id = ?
    ...
    `).bind(r).run(),console.log(`✅ Chauffeur ${r} clôturé`),t.json({success:!0})}catch(r){return console.error("Erreur clôture chauffeur:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeur/notification",async t=>{try{const{chauffeur_id:r,type:s,titre:i,message:a}=await t.req.json();return await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(r,s,i,a).run(),t.json({success:!0})}catch(r){return console.error("Erreur création notification:",r),t.json({success:!1,error:r.message},500)}});p.post("/api/chauffeurs/inscription",$i);p.get("/api/chauffeurs/:id",Vi);p.get("/api/chauffeurs/:id/taches",Gi);p.post("/api/taches/:id/completer",Qi);p.get("/api/chauffeurs/:id/messages",Wi);p.post("/api/messages",Xi);p.post("/api/messages/:id/marquer-vu",Yi);p.get("/api/admin/chauffeurs-actifs",Zi);p.post("/api/admin/taches/assigner",Ji);p.post("/api/controleur/improd",async t=>{try{const r=await t.req.json();if(console.log("📋 Improductivité reçue:",r),!r.controleur_nom||!r.raison||!r.duree)return t.json({success:!1,error:"Données manquantes"},400);await t.env.DB.prepare(`
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);let l=null,o=null;try{const u=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();if(console.log("📊 Quai data AVANT UPDATE:",u),l=u==null?void 0:u.timer_start,u!=null&&u.timer_start){const h=await t.env.DB.prepare(`
          SELECT 
            CAST((julianday('now') - julianday(?)) * 86400 AS INTEGER) as raw_duration
        `).bind(u.timer_start).first(),f=(h==null?void 0:h.raw_duration)||0;o=f>=3600?f-3600:f,console.log(`⏱️ DÉCHARGEMENT: timer_start=${u.timer_start}, Brut=${f}s, Corrigé=${o}s (${Math.floor(o/60)}min ${o%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:o,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
