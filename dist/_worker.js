...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function Ui(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a||null).run();return t.json({success:!0,id:n.meta.last_row_id,message:"Tâche assignée"})}catch(r){return console.error("Erreur assignation tâche:",r),t.json({error:"Erreur serveur"},500)}}const g=new Rr;g.use("/static/*",Ws({root:"./",onNotFound:(t,r)=>(console.log("Fichier non trouvé:",t),r.notFound())}));g.get("/login",Ve,t=>t.render(e(Di,{})));g.get("/qrcode-chauffeur",Ve,t=>t.render(e(ji,{})));g.get("/chauffeur/langue",Ve,t=>t.render(e(Pi,{})));g.get("/gestion-quais",Ve,t=>t.render(e(Ii,{})));g.get("/pdf-barcodes",t=>t.html(zi()));g.get("/qrcode-generator",t=>t.html(Bi()));g.get("/scan",t=>{const r=t.req.query("quai"),s=t.req.query("action")||"start";return r?t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
  ...
  `)});g.get("/scan-controle",async t=>{const r=t.req.query("quai");if(!r)return t.html(`
    ...
    `)}});g.get("/download-qr-controle",t=>t.html(`<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`));g.get("/download-qr-fin-controle",t=>t.html(`<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_chauffeur_id ON chat_messages(chauffeur_id)").run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)").run(),console.log("✅ Table chat_messages prête avec colonnes de traduction")}catch(r){console.error("⚠️ Erreur auto-migration chat_messages:",r.message)}}g.post("/api/chauffeur/chat",async t=>{try{await zr(t.env.DB);const{chauffeur_id:r,message:s,sender:i,lang:a}=await t.req.json();if(!r||!s)return t.json({success:!1,error:"Données manquantes"},400);let n=a||"fr";if(!a){const m=await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(r,l,s,d,o,c,l==="chauffeur"?0:1,l==="admin"?0:1).run(),console.log(`✅ [CHAT] Message enregistré avec succès - ID chauffeur: ${r}`),t.json({success:!0,translated_fr:o,translated_chauffeur:c})}catch(r){return console.error("Erreur envoi message:",r),t.json({success:!1,error:r.message},500)}});g.get("/api/chauffeur/chat",async t=>{try{await zr(t.env.DB);const r=t.req.query("id")||t.req.query("chauffeur_id"),s=t.req.query("viewer")||"chauffeur";console.log(`📥 [CHAT GET] Récupération messages - Chauffeur: ${r}, Viewer: ${s}`);const i=await t.env.DB.prepare(`
      SELECT langue FROM chauffeur_arrivals WHERE id = ?
    ...
    `).bind(r).run(),console.log(`✅ Chauffeur ${r} clôturé`),t.json({success:!0})}catch(r){return console.error("Erreur clôture chauffeur:",r),t.json({success:!1,error:r.message},500)}});g.post("/api/chauffeur/notification",async t=>{try{const{chauffeur_id:r,type:s,titre:i,message:a}=await t.req.json();return await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).bind(r,s,i,a).run(),t.json({success:!0})}catch(r){return console.error("Erreur création notification:",r),t.json({success:!1,error:r.message},500)}});g.post("/api/chauffeurs/inscription",Ui);g.get("/api/chauffeurs/:id",$i);g.get("/api/chauffeurs/:id/taches",Vi);g.post("/api/taches/:id/completer",Hi);g.get("/api/chauffeurs/:id/messages",Gi);g.post("/api/messages",Qi);g.post("/api/messages/:id/marquer-vu",Xi);g.get("/api/admin/chauffeurs-actifs",Wi);g.post("/api/admin/taches/assigner",Yi);g.get("/api/quais",async t=>{try{const{results:r}=await t.env.DB.prepare(`
