...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:i,controleurNom:s}=r;if(!i||!s)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(i).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")).getTime(),o=new Date(Hr()).getTime();n=Math.floor((o-l)/1e3),console.log(`⏱️ Durée contrôle calculée: ${n}s`)}await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_chauffeur_id ON chat_messages(chauffeur_id)").run(),await t.prepare("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)").run(),console.log("✅ Table chat_messages prête avec colonnes de traduction")}catch(r){console.error("⚠️ Erreur auto-migration chat_messages:",r.message)}}p.post("/api/chauffeur/chat",async t=>{try{await Ur(t.env.DB);const{chauffeur_id:r,message:i,sender:s,lang:a}=await t.req.json();if(!r||!i)return t.json({success:!1,error:"Données manquantes"},400);let n=a||"fr";if(!a){const u=await t.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(r,l,i,d,o,c,l==="chauffeur"?0:1,l==="admin"?0:1).run(),console.log(`✅ [CHAT] Message enregistré avec succès - ID chauffeur: ${r}`),t.json({success:!0,translated_fr:o,translated_chauffeur:c})}catch(r){return console.error("Erreur envoi message:",r),t.json({success:!1,error:r.message},500)}});p.get("/api/chauffeur/chat",async t=>{try{await Ur(t.env.DB);const r=t.req.query("id")||t.req.query("chauffeur_id"),i=t.req.query("viewer")||"chauffeur";console.log(`📥 [CHAT GET] Récupération messages - Chauffeur: ${r}, Viewer: ${i}`);const s=await t.env.DB.prepare(`
      SELECT langue FROM chauffeur_arrivals WHERE id = ?
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,s).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);let l=null,o=null;try{const d=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();if(console.log("📊 Quai data AVANT UPDATE:",d),l=d==null?void 0:d.timer_start,d!=null&&d.timer_start){const u=new Date(d.timer_start.replace(" ","T")).getTime(),h=new Date(Hr()).getTime();o=Math.floor((h-u)/1e3),console.log(`⏱️ Durée calculée: ${o}s (${Math.floor(o/3600)}h ${Math.floor(o%3600/60)}m ${o%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:o,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
