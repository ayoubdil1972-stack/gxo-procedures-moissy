...
<body class="bg-gradient-to-br from-blue-50 to-purple-50 min-h-screen p-8">
  ...
  `}async function Vi(t){try{const{nom:r,entreprise:s,telephone:i,plaque:a,type_camion:n,langue:l}=await t.req.json();if(!r||!s||!i||!a||!n)return t.json({error:"Tous les champs sont obligatoires"},400);const c=(await t.env.DB.prepare(`
    ...
    `).bind(r,s,i,a||null).run();return t.json({success:!0,id:n.meta.last_row_id,message:"Tâche assignée"})}catch(r){return console.error("Erreur assignation tâche:",r),t.json({error:"Erreur serveur"},500)}}const p=new Sr;function Hr(){const t=new Date;return new Date(t.toLocaleString("en-US",{timeZone:"Europe/Paris"})).toISOString()}p.use("/static/*",Ys({root:"./",onNotFound:(t,r)=>(console.log("Fichier non trouvé:",t),r.notFound())}));p.get("/login",$e,t=>t.render(e(Ii,{})));p.get("/qrcode-chauffeur",$e,t=>t.render(e(Pi,{})));p.get("/chauffeur/langue",$e,t=>t.render(e(Li,{})));p.get("/gestion-quais",$e,t=>t.render(e(Bi,{})));p.get("/pdf-barcodes",t=>t.html(zi()));p.get("/qrcode-generator",t=>t.html($i()));p.get("/scan-dechargement",t=>{const r=t.req.query("quai");return t.redirect(`/scan?quai=${r}`)});p.get("/scan",t=>{const r=t.req.query("quai"),s=t.req.query("action")||"start";return r?t.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
      <meta charset="UTF-8">
  ...
  `):t.html(`
    ...
    `)});p.post("/api/fin-controle",async t=>{try{const r=await t.req.json(),{quai:s,controleurNom:i}=r;if(!s||!i)return t.json({error:"Données manquantes"},400);const a=await t.env.DB.prepare(`
      ...
      FROM quai_status 
      WHERE quai_numero = ?
    `).bind(s).first();let n=null;if(a!=null&&a.timer_controle_start){const l=new Date(a.timer_controle_start.replace(" ","T")).getTime(),o=new Date(Hr()).getTime();let c=Math.floor((o-l)/1e3);c>=3600?(n=c-3600,console.log(`⏱️ Durée contrôle AVANT correction: ${c}s → APRÈS: ${n}s (-1h)`)):n=c,console.log(`⏱️ Durée contrôle finale: ${n}s`)}await t.env.DB.prepare(`
      UPDATE quai_status 
...
<body class="bg-gradient-to-br from-purple-50 to-purple-100 min-h-screen p-8">
    ...
    `).bind(r.quai_numero,r.nom_agent,r.palettes_attendues,r.palettes_recues,r.palettes_a_rendre,a,r.autres_commentaire||null,i).run();console.log("✅ Fin de déchargement enregistrée - ID:",n.meta.last_row_id);try{const o=await t.env.DB.prepare(`
      ...
      `).bind(r.quai_numero).first();console.log("📊 Quai data AVANT UPDATE:",o);const c=o==null?void 0:o.timer_start;let d=null;if(o!=null&&o.timer_start){const u=new Date(o.timer_start.replace(" ","T")).getTime(),h=new Date(Hr()).getTime();let f=Math.floor((h-u)/1e3);f>=3600?(d=f-3600,console.log(`⏱️ Durée AVANT correction: ${f}s → APRÈS: ${d}s (-1h)`)):d=f,console.log(`⏱️ Durée finale: ${d}s (${Math.floor(d/3600)}h ${Math.floor(d%3600/60)}m ${d%60}s)`)}console.log("💾 UPDATE avec:",{timerDuration:d,commentaire:`Déchargement terminé - ${r.nom_agent} - ${r.fournisseur} - ID:${r.numero_id}`,commentaire_auteur:r.nom_agent,quai_numero:r.quai_numero}),await t.env.DB.prepare(`
        UPDATE quai_status 
